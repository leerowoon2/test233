	...
	this.record = function(event) {
		...
		eventjs.proxy.drag({
			...
			listener: function(event, self) {
				//console.log($("#editView").scrollTop());
				eventjs.cancel(event);
				///
				var coords = {};
				var scale = $("#currentSlide").attr("kk_zoom");
				var offset_t = $(layer2).offset().top;
				var offset_l = $(layer2).offset().left;

				console.log(self.x +"----------"+scale);
				//(e.pageX/scale -layer2.offset().left),
			    coords.x = ((self.x + offset_l)/scale - offset_l) - $("#editView").scrollLeft()/scale;
                //coords.y = ((self.y + offset_t)/scale - offset_t) - $("#editView").scrollTop()/scale;
				//coords.x = self.x * 1.0 / scale;
				coords.y = self.y * 1.0 / scale;
				///
